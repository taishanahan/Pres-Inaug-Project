...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="rep_docs" select="collection('.?select=*.xml')"/>
    ...
    <xsl:template match="/">
        <svg height="1500" width="1500" viewBox="-100, -650, 1500, 1500">
            ...
            <xsl:for-each select="1 to 10">
                <line x1="{$x1}" x2="{$x2}" y1="-{(. * 10) * $multiple}" y2="-{(. * 10) * $multiple}" stroke="grey" stroke-linecap="square" stroke-dasharray="10,10"/>
                <text x="-25" y="-{(. * 10) * $multiple - 5}" stroke="black">
